...
export const appointmentAlreadyExists = async (appointmet: Appointment) => {
    ...
    let existing_appointment: any = await dbQuerry('SELECT * from appointments WHERE client=' + String(appointmet.client) +' AND staff_member=' + String(appointmet.staff_member)+ ' AND start=' + String(appointmet.start))
